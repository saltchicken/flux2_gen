...
import io
import sys
# NOTE: Assuming Flux2Pipeline exists in your specific diffusers fork
...
class FluxGenerator:
    ...
    def _remote_text_encoder(self, prompt):
        ...
        # If you MUST use pickle, we keep weights_only=False but warn.
        return torch.load(io.BytesIO(response.content), weights_only=False).to(self.device)

    def generate(self, prompt, steps=50, guidance=4.0, seed=None):
